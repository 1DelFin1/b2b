...
class Authorization:
    ...
    async def login(session: AsyncSession, email: str, password: str) -> TokenResponse:
        ...
        seller = await SellerService.get_by_email(session, email)
        if seller is None or not verify_password(password, seller.hashed_password):
        ...
        refresh_token = await RefreshTokenService.create(session, str(seller.id), "seller")

        return TokenResponse(
            access_token=access_token,
            refresh_token=refresh_token,
            expires_in=settings.jwt.ACCESS_TOKEN_EXPIRE_MINUTES * 60,
            user_id=UUID(str(seller.id)),
        )
